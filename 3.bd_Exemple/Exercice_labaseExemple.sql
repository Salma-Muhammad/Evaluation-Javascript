...
--les colonnes
SELECT CONCAT (nom ,"  ", prenom) AS "Nom des Employés"
FROM employe
ORDER BY nom


--29. Afficher les 5 premières lettres du nom des employés.
SELECT SUBSTRING (nom, 1,5) AS "Nom des Employés"
FROM employe


--30. Afficher le nom et le rang de la lettre « r » dans le nom des
--employés.
SELECT LOCATE ('r',nom) AS "Nom des Employés"
FROM employe
ORDER BY nom
...
SELECT UPPER (nom)AS "NOM", LOWER (nom) AS "nom"
FROM employe
WHERE nom= "vrante"


--32. Afficher le nom et le nombre de caractères du nom des employés
SELECT nom, LENGTH(nom) AS "Nb de caractères", 
FROM employe